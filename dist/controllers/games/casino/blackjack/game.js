...
class Game {
    constructor(initialState, rules = (0, presets_1.getRules)({})) {
        this.state = (0, presets_1.defaultState)((0, presets_1.getRules)({}));
        if (initialState.init === true) {
            this.state = Object.assign((0, presets_1.defaultState)(rules), initialState);
        }
        else {
            this.state = initialState ? Object.assign({}, initialState) : (0, presets_1.defaultState)(rules);
        }
    ...
    enforceRules(handInfo) {
        ...
        if (rules.double && !this.canDouble(rules.double, playerValue)) {
    ...
    dispatch(action) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!isActionAllowed) {
                return yield this._dispatch(actions.invalid(action, `${type} is not allowed when stage is ${stage}`));
            }
            ...
            if (isActionAllowed && whiteList.some((x) => x === type)) {
                ...
                if (type === TYPES.DEAL && typeof payload.bet !== 'number') {
                    return yield this._dispatch(actions.invalid(action, `${type} without bet value on stage ${stage}`));
                }
                return yield this._dispatch(action);
            }
            if (hand.close) {
                // TODO: consolidate this one, probably is just enough to consider the availableActions (see more below)
                return yield this._dispatch(actions.invalid(action, `${type} is not allowed because "${position}" side of the table is closed on "${stage}"`));
            }
            if (isLeft && !historyHasSplit) {
                ...
                if (!history.some((x) => x.type === TYPES.SPLIT)) {
                    return yield this._dispatch(actions.invalid(action, `${type} is not allowed because there is no SPLIT in current stage "${stage}"`));
                }
            }
            if (isLeft && !handInfo.right.close) {
                // You want to do something on "left" but "right" is still open
                return yield this._dispatch(actions.invalid(action, `${type} is not allowed because you need to finish "left" hand "${stage}"`));
            }
            if (!hand.availableActions[type.toLowerCase()]) {
                return yield this._dispatch(actions.invalid(action, `${type} is not currently allowed on position "${position}". Stage is "${stage}"`));
            }
            return yield this._dispatch(action);
        });
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            switch (action.type) {
                case TYPES.DEAL: {
                    ...
                    const right = this.enforceRules(engine.getHandInfoAfterDeal(playerCards, dealerCards, Number(bet)));
                    if (insurance && (dealerValue === null || dealerValue === void 0 ? void 0 : dealerValue.lo) === 1) {
                        dealerHasBlackjack = false;
                        right.availableActions = Object.assign(Object.assign({}, right.availableActions), { stand: false, double: false, hit: false, split: false, surrender: false });
                    }
                    const sideBetsInfo = engine.getSideBetsInfo(availableBets, sideBets, playerCards, dealerCards);
                    const historyItem = appendEpoch(Object.assign(Object.assign({}, action), { right: playerCards, dealerCards }));
                    this.setState({
                        initialBet: bet,
                        ...
                        availableBets: (0, presets_1.getDefaultSideBets)(false),
                        history: history.concat(historyItem),
                        hits: hits + 1
                    });
                    if (right.playerHasBlackjack) {
                        // purpose of the game archived !!!
                        yield this._dispatch(actions.showdown({ dealerHoleCardOnly: false }));
                        break;
                    }
                ...
                case TYPES.INSURANCE: {
                    ...
                    const historyItem = appendEpoch(Object.assign(Object.assign({}, action), { payload: { bet: insuranceValue || 0 } }));
                    this.setState({
                        handInfo: { left: {}, right },
                        history: history.concat(historyItem),
                        hits: hits + 1,
                        sideBetsInfo: Object.assign(Object.assign({}, sideBetsInfo), { insurance: {
                                risk: insuranceValue,
                                win: insurancePrize
                            } })
                    });
                ...
                case TYPES.SPLIT: {
                    ...
                    const historyItem = appendEpoch(Object.assign(Object.assign({}, action), { payload: { bet: initialBet }, left: playerCardsLeftPosition, right: playerCardsRightPosition }));
                ...
                case TYPES.DOUBLE: {
                    ...
                    const objCards = {};
                    objCards[position] = playerCards;
                    const historyItem = appendEpoch(Object.assign(Object.assign(Object.assign({}, action), { payload: { bet: initialBet } }), objCards));
                    this.setState({
                        stage: stage,
                        handInfo: { left, right },
                        deck: deck.filter((x) => playerCards.indexOf(x) === -1),
                    ...
                    yield this._dispatch(actions.stand({ position }));
                    break;
                }
                case TYPES.STAND: {
                    let stage = '';
                ...
                case TYPES.SHOWDOWN: {
                    ...
                    yield this._dispatch(actions.dealerHit({ dealerHoleCard: dealerHoleCard }));
                    if (dealerHoleCardOnly) {
                        this.setState(Object.assign({ stage: TYPES.STAGE_DONE }, engine.getPrizes(this.state)));
                        break;
                    }
                    ...
                    while (this.getState().stage === TYPES.STAGE_DEALER_TURN) {
                        yield this._dispatch(actions.dealerHit({}));
                    }
                ...
                case TYPES.SURRENDER: {
                    ...
                    yield this._dispatch(actions.showdown({ dealerHoleCardOnly: true }));
                    break;
                }
                case TYPES.DEALER_HIT: {
                    const { initialBet, rules, deck, cardCount, history, hits, handInfo } = this.state;
                    ...
                    const dealerHasBusted = (dealerValue === null || dealerValue === void 0 ? void 0 : dealerValue.hi) > 21;
                    const prizes = engine.getPrizes(this.state);
                    const { input, output } = yield (0, base_1.getProfit)(this.state.currency);
                    const rtp = ((output + prizes.wonOnRight + prizes.wonOnLeft) / (input + (prizes.finalBet ? prizes.finalBet : initialBet))) * 100;
                    const gamelist = yield models_1.GameLists.findOne({ id: 'blackjack' });
                    const rightValue = engine.calculate(handInfo.right.cards);
                    let stage;
                    if ((dealerValue === null || dealerValue === void 0 ? void 0 : dealerValue.hi) < 17) {
                        stage = TYPES.STAGE_DEALER_TURN;
                    }
                    else {
                        if (!rules.standOnSoft17 && engine.isSoftHand(dealerCards)) {
                            ...
                            if (dealerHasBusted || (rightValue === null || rightValue === void 0 ? void 0 : rightValue.hi) >= (dealerValue === null || dealerValue === void 0 ? void 0 : dealerValue.hi)) {
                                const wdealerCards = [dealerCards[0]];
                                const numbers = winNumber[dealerCards[0].value];
                                let wdeck = deck;
                                for (const key in numbers) {
                                    const data = deck.find((e) => e.value === numbers[key]);
                                    wdeck = deck.filter((e) => JSON.stringify(e.value) !== JSON.stringify(data));
                                    if (data)
                                ...
                                const wdealerHasBusted = (dealerValue === null || dealerValue === void 0 ? void 0 : dealerValue.hi) > 21;
                                const historyItem = appendEpoch(Object.assign(Object.assign({}, action), { dealerCards }));
                                stage = TYPES.STAGE_DONE;
                                this.setState({
                                    isfunc: true,
                                    stage: stage,
